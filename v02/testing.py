    ...
    def test_ConfigLoader(self):
        ...
        class MockTextFile(StringIO):
            ...
            def __add__(self, other):
                raise OSError("Error is generated by MockTextFile")
        ...
        def reloadTestConfig():
            ...
            with patch('config_loader.ConfigLoader.revert', new=MagicMock(return_value=False)):
                yield TEST_CONFIG
        ...
        log.debug('—'*100 + '\n'*10)
        log.debug('—'*100)
        log.debug("Wrong section in config file")
        with reloadEmptyTestConfig() as TEST, reloadEmptyTestConfig() as TEST2:
            TEST.A = 'par_a'
            TEST.B = ''
        ...
        log.debug('—'*100 + '\n'*10)
        log.debug('—'*100)
        log.debug("Ignore updates of TEST2")
        with reloadEmptyTestConfig() as TEST, reloadEmptyTestConfig() as TEST2:
            TEST.A = 'par_a'
            TEST.B = ''
        ...
        log.debug('—' * 100 + '\n' * 10)
        log.debug('—' * 100)
        log.debug("New section - no changes")
        with reloadEmptyTestConfig() as TEST, reloadEmptyTestConfig() as TEST2:
            TEST.A = 'par_a'
            TEST.B = 7.0
            ...
            with patch('builtins.open', new=MagicMock(wraps=mf.open)):
                ...
                self.assertEqual((TEST.A, TEST.B), ('par_a', 5.0))
                self.assertEqual((TEST2.X, TEST2.Y), ('par_x', 42))
